...
source("nesterov_low_rank_plus_sparse.R")
require(ggplot2)
...
for (gamma in gammas) {
  ...
  for (lambda in lambdas) {
    ...
    results <- rbind(results, c(lambda, gamma, prec, rec, rank))
    print(paste("Lambda:", lambda, ", #Iterations:", fit$iter, ", Sparsity:", sparsity, ", Rank:", rank))
    if(sparsity == 0) {
      break()
    }
  }
...
names(df.results) <- c("Lambda", "Gamma", "Precision", "Recall", "Rank")
